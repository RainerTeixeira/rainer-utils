...
var STRENGTH_LABELS = {
  "pt-BR": ["Muito fraca", "Fraca", "M\xE9dia", "Forte", "Muito forte"],
  "en-US": ["Very weak", "Weak", "Fair", "Strong", "Very strong"],
  "es-ES": ["Muy d\xE9bil", "D\xE9bil", "Regular", "Fuerte", "Muy fuerte"]
};
var STRENGTH_COLORS = [
  "bg-red-500",
  "bg-orange-500",
...
function usePasswordStrength(password, locale = DEFAULT_LOCALE) {
  if (!password) {
...
function useCarouselKeyboard({
  ...
  const [currentSlide, setCurrentSlide] = React__default.default.useState(
    Math.min(Math.max(0, initialSlide), slideCount - 1)
  );
  const [isPlaying, setIsPlaying] = React__default.default.useState(autoplay);
  const [prefersReducedMotion2, setPrefersReducedMotion] = React__default.default.useState(false);
  const autoplayTimerRef = React__default.default.useRef(null);
  ...
  const goToSlide = React__default.default.useCallback(
    (index) => {
  ...
  const goToNext = React__default.default.useCallback(() => {
    let nextIndex;
  ...
  const goToPrevious = React__default.default.useCallback(() => {
    let prevIndex;
    if (currentSlide === 0) {
  ...
  const goToFirst = React__default.default.useCallback(() => goToSlide(0), [goToSlide]);
  const goToLast = React__default.default.useCallback(() => goToSlide(slideCount - 1), [goToSlide, slideCount]);
  const pauseAutoplay = React__default.default.useCallback(() => {
    if (isPlaying) {
      setIsPlaying(false);
      onAutoplayToggle?.(false);
    }
  }, [isPlaying, onAutoplayToggle]);
  const resumeAutoplay = React__default.default.useCallback(() => {
    if (autoplay && !isPlaying && (!respectReducedMotion || !prefersReducedMotion2)) {
      setIsPlaying(true);
  ...
  const toggleAutoplay = React__default.default.useCallback(() => {
    if (isPlaying) {
      pauseAutoplay();
    } else {
      resumeAutoplay();
  ...
  React__default.default.useEffect(() => {
    ...
    if (respectReducedMotion && prefersReducedMotion2) {
      setIsPlaying(false);
      return;
    }
    if (isPlaying) {
...
function useTableOfContents({
  ...
  React.useEffect(() => {
    ...
    const generateUniqueId2 = (text, index, level, usedIds2) => {
      const slug = text.toLowerCase().trim().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").substring(0, 50);
      const baseId = slug ? `heading-${slug}-${level}-${index}` : `heading-${level}-${index}`;
      let uniqueId = baseId;
      let counter = 0;
      while (usedIds2.has(uniqueId)) {
        counter++;
    ...
    elements.forEach((element, index) => {
      ...
      if (!id || usedIds.has(id)) {
        id = generateUniqueId2(text, index, level, usedIds);
        element.id = id;
      }
      usedIds.add(id);
